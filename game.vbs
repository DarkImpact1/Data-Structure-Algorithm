...
' Logging function (optional, can remove if not needed)
Sub Log(msg)
    WScript.Echo msg
End Sub
...
Function CommandExists(cmd)
    ...
    CommandExists = (InStr(LCase(output), LCase(cmd)) > 0)
End Function
...
If Not CommandExists("git") Then
    Log "Git not found. Downloading and installing Git silently..."
...
' Re-check Git existence after installation
If Not CommandExists("git") Then
    Log "Git installed but not available in current PATH. Restarting script..."
    WshShell.Run "wscript.exe " & Chr(34) & scriptPath & Chr(34), 0
    WScript.Quit
End If

' Install Python if not present
If Not CommandExists("python") Then
    Log "Python not found. Downloading and installing Python silently..."
